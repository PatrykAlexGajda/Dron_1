...
template std::istream & operator >> (std::istream & Strm, Macierz<double, 3> & k);
template std::ostream & operator << (std::ostream & Strm, const Macierz<double, 3> & k);
...
template <class TYP, int ROZMIAR>
Macierz<TYP, ROZMIAR> Macierz<TYP, ROZMIAR>::operator * (const double & d) const{

    Macierz wynik;

    for(int i=0;i<ROZMIAR;i++){
        for(int j=0;j<ROZMIAR;j++){
            wynik.tab[i][j] = tab[i][j] * d;
        }
    }
    return wynik;
}
